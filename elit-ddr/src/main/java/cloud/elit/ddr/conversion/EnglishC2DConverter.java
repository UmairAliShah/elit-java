...
//	============================= Abstract Methods =============================
    ...
    public Sentence toDependencyGraph(CTTree tree) {
        ...
        Sentence graph = createDependencyGraph(tree);
        relabel(tree, graph);
        addFeats(tree, graph);
        validate(tree, graph);
        return graph;
...
//	============================= Pre-process =============================
    ...
    public void preprocess(CTTree tree) {
        ...
        tree.getTerminals().forEach(n -> preprocessEmptyCategory(tree, n));
        tree.getRoot().getTerminals().stream().filter(this::removeEmptyCategoriesAux).forEach(Node::removeSelf);
    }
    ...
    public void preprocessModalVerb(CTNode node) {
    ...
    public void preprocessModalAdjective(CTNode node) {
    ...
    public void preprocessQuantifierPhrase(CTNode node) {
        if (!node.isSyntacticTag(PTBTag.C_QP)) return;
    ...
    private void preprocessSQ(CTNode node) {
...
//	============================= Pre-process: Empty Categories =============================
    ...
    private void preprocessDuplicates(CTTree tree) {
        for (Entry<Integer, List<CTNode>> e : tree.getEmptyCategoryMap().int2ObjectEntrySet()) {
            ...
            CTNode ante = Objects.requireNonNull(node).getAntecedent();
        ...
        if (node.isEmptyCategory()) {
            switch (node.getForm()) {
                case PTBTag.E_ZERO:
                    break;
                case PTBTag.E_NULL:
                    preprocessPassiveNull(tree, node);
                    break;
                case PTBTag.E_TRACE:
                    preprocessTrace(node);
                    break;
                case PTBTag.E_PRO:
                    preprocessPRO(node);
                    break;
                case PTBTag.E_EXP:
                    preprocessEXP(tree, node);
                    break;
                case PTBTag.E_ICH:
                case PTBTag.E_PPA:
                case PTBTag.E_RNR:
                    preprocessDiscontinuousConstituent(tree, node);
                    break;
                default:
                    node.removeSelf();
            }
...
//	============================= Coordination =============================
    ...
    private CTNode findHeadCoordination(CTNode curr, HeadRule rule) {
        ...
        int i, sId, size = curr.numChildren();
        CTNode node;
    ...
    private boolean isConjunct(CTNode C, CTNode P, Pattern rTags) {
        ...
        if (rTags.matcher(C.getSyntacticTag()).find()) {
            return (!P.isSyntacticTag(PTBTag.C_VP) || getAuxiliaryLabel(C) == null) && (!PTBLib.isMainClause(P) || !C.isSyntacticTag(PTBTag.C_S) || !hasAdverbialTag(C));
        }

        return P.isSyntacticTag(PTBTag.C_NP) && C.isFunctionTag(PTBTag.F_NOM);
    }
    ...
    private void handleArgumentsInCoordination(CTNode curr, List<CTNode> heads) {
        ...
        while (curr.hasParent(PTBLib::isVerbPhrase) || curr.hasParent(n -> n.isSyntacticTag(PTBTag.C_S))) {
            curr.getSiblings().stream().filter(this::isCoordArgument).forEach(n -> n.addSecondaryHeads(sub));
            curr = curr.getParent();
...
//	============================= Find Heads =============================
     ...
     */
    private void findHeadApposition(CTNode curr) {
        ...
        if (fst == null || fst.isEmptyCategoryPhrase() || fst.hasPrimaryHead()) return;
    ...
    private void findHeadQuantifierPhrase(CTNode node) {
        if (!node.isSyntacticTag(PTBTag.C_QP)) return;
...
// ============================= Get Labels =============================
    ...
    private boolean isClausalComplement(CTNode node) {
        return node.isSyntacticTag(CCOMP) || node.isSyntacticTag(PTBTag.C_SBAR) && (node.containsChild(PTBLib::isNullComplementizer) || node.containsChild(n -> n.isSyntacticTag(MARK)) || node.isFunctionTag(DDGTag.RELCL) || node.containsChild(PTBLib::isWhPhrase));
    }

    private boolean isOpenClausalComplement(CTNode node) {
        if (node.isSyntacticTag(PTBTag.C_S)) {
            CTNode sbj = node.getFirstChild(PTBLib::isSubject);
            return node.containsChild(PTBLib::isVerbPhrase) && (sbj == null || sbj.isEmptyCategoryPhrase());
    ...
    private boolean isPossive(CTNode node, CTNode parent) {
        return node.isSyntacticTag(POSS) || parent.isSyntacticTag(POSS_PARENT) && node.containsChild(n -> n.isSyntacticTag(PTBTag.P_POS));
    }
    ...
    private boolean isDiscourse(CTNode node, CTNode d) {
        return node.isSyntacticTag(PTBTag.C_EDITED) || node.isSyntacticTag(PTBTag.C_EMBED) || node.isSyntacticTag(PTBTag.C_META) || node.isSyntacticTag(INTJ) || d.isSyntacticTag(INTJ) || d.isSyntacticTag(PTBTag.P_EMO) || emoticon.isEmoticon(d.getForm());
    }
    ...
    private boolean isAdverb(CTNode node, CTNode d) {
        return (node.isSyntacticTag(PTBTag.C_ADVP) || PTBLib.isAdverb(node) || PTBLib.isAdverb(d)) && (!node.hasParent(n -> n.isSyntacticTag(PP)) || node.hasRightSibling() || !node.getLeftNearestSibling().isSyntacticTag(PREP));
    }
    ...
    private boolean isNegation(CTNode node) {
        return PTBLib.isAdverb(node) && node.getForm() != null && ENUtils.isNegation(node.getForm()) || node.isSyntacticTag(PTBTag.C_ADVP) && node.matchesForms("no", "longer");
    }
...
//					if (PTBLib.isSubject(np) && !PTBLib.isSubject(ante))
//						ante.addSecondaryHead(arc);
//					else if (!node.isDescendantOf(ante) && !isPrimaryAncestorOf(ante, arc.get()))
...
// ============================= relabel =============================
    ...
    private void relabel(CTTree tree, Sentence graph) {
        graph.forEach(n -> relabel(tree, n));
        labelCompounds(graph);
        labelNonVerbalPredicates(tree, graph);
    }

    private void relabel(CTTree tree, NLPNode node) {
        if (node.getPartOfSpeechTag().startsWith(PTBLib.P_VB))
            relabelEXPL(node);
        else if (node.isPartOfSpeechTag(PTBTag.P_IN)) {
        ...
        Iterator<NLPArc> it = node.getSecondaryParents().iterator();
    ...
    private String relabelAux(NLPNode head, String label) {
        ...
        if (label.equals(DDGTag.COP) && head.getPartOfSpeechTag().startsWith(PTBLib.P_VB))
            return DDGTag.AUX;
    ...
    private void labelCompounds(Sentence graph) {
        labelCompounds(graph,
                head -> head.getPartOfSpeechTag().startsWith(PTBLib.P_NN),
                node -> node.isDependencyLabel(DDGTag.ATTR) || node.isDependencyLabel(DDGTag.DEP) || node.getPartOfSpeechTag().startsWith(PTBTag.P_NNP));
        ...
        labelCompounds(graph,
                head -> head.getPartOfSpeechTag().startsWith(PTBLib.P_RB) || head.getPartOfSpeechTag().equals(PTBLib.P_WRB),
                node -> node.isDependencyLabel(DDGTag.ADV) || node.isDependencyLabel(DDGTag.DEP) || node.isDependencyLabel(DDGTag.NEG));
    }

    private void labelCompounds(Sentence graph, Predicate<NLPNode> main_condition, Predicate<NLPNode> node_condition) {
        NLPNode node, head;
        int i, j;
            ...
            if (main_condition.test(head)) {
                for (j = i - 1; j >= 0; j--) {
                    ...
                    if (node.isDescendantOf(head) && node.getParent().compareTo(node) > 0 && main_condition.test(node) && node_condition.test(node)) {
                        ...
                        i = j;
                    } else if (node.isPartOfSpeechTag(PTBTag.P_HYPH) && !node.isDependencyLabel(DDGTag.CC))
                        continue;
                    else
                        break;
                }
    ...
    private boolean isNumber(NLPNode node) {
        return node.isPartOfSpeechTag(PTBLib.P_CD) || node.isLemma("0") || node.isLemma(MetaConst.CARDINAL) || node.isLemma(MetaConst.ORDINAL);
    }

    private void labelNonVerbalPredicates(CTTree tree, Sentence graph) {
        for (NLPNode node : graph) {
            if (node.getPartOfSpeechTag().startsWith(PTBLib.P_VB) && light_verbs.contains(node.getLemma()) && !node.containsChild(n -> n.isDependencyLabel(DDGTag.PRT)))
            {
                NLPNode obj = node.getFirstChild(n -> n.isDependencyLabel(DDGTag.OBJ));

                if (obj != null && eventive_nouns.contains(obj.getLemma()) && !node.containsChild(n -> n.isDependencyLabel(DDGTag.OPRD))) {
                    // heads
                    ...
                    for (NLPNode d : node.getChildren()) {
                        if (d.isDependencyLabel(DDGTag.DAT))
                            d.setDependencyLabel(DDGTag.OBJ);
                        else if (d.isDependencyLabel(DDGTag.PPMOD))
                            relabelPPMod(obj, d, n -> n.isLemma("of") || n.isLemma("to"));
                    }

                    for (NLPNode d : obj.getChildren()) {
            ...
            else if (node.getPartOfSpeechTag().startsWith(PTBLib.P_JJ) && node.containsChild(n -> n.isDependencyLabel(DDGTag.COP))) {
                NLPNode ppmod = node.getFirstChild(n -> n.isDependencyLabel(DDGTag.PPMOD));
                if (ppmod != null) relabelPPMod(node, ppmod, n -> n.isLemma("of"));
            }
        }
    }

    private void relabelPPMod(NLPNode predicate, NLPNode ppmod, Predicate<NLPNode> pr) {
        NLPNode p = ppmod.getFirstChild(pr);

        if (p != null && predicate.getTokenID() + 1 == p.getTokenID()) {
            String tag = ppmod.getPartOfSpeechTag();

            if (tag.startsWith(PTBLib.P_NN) || tag.equals(PTBLib.P_PRP) || tag.equals(PTBLib.P_WP))
                ppmod.setDependencyLabel(DDGTag.OBJ);
            else
                ppmod.setDependencyLabel(DDGTag.COMP);
        }
    }
    ...
    private void addFeats(CTTree tree, Sentence graph) {
        tree.flatten().forEach(n -> addFeats(n, graph));
    }

    private void addFeats(CTNode node, Sentence graph) {
        ...
        if (!tags.isEmpty()) {
            String feat = StringUtils.toLowerCase(Joiner.join(tags, ","));
            NLPNode n = graph.get(term.getTokenID());
    ...
    public void validate(CTTree tree, Sentence graph) {
        String message = null;

        if (graph.getRoot().numChildren() != tree.getRoot().numChildren())
        ...
        for (NLPNode node : graph) {
            ...
            for (NLPArc arc : node.getSecondaryParents()) {
...
//		for (T arc : heads)
//		{
//			if (arc.get() == null || set.contains(arc.get().getTokenID()))
//				remove.add(arc);
//		}
//		
...
////		{
////			if (rnr.get().isArgumentOf(sHead, label))
////				return true;
////		}
////		
////		return false;
