...
public class EnglishC2DConverter extends C2DConverter {
    ...
    private final Set<String> META = Set.of(PTBTag.C_EDITED, PTBTag.C_EMBED, PTBTag.C_LST, PTBTag.C_META, PTBTag.P_CODE, PTBTag.C_CAPTION, PTBTag.C_CIT, PTBTag.C_HEADING, PTBTag.C_TITLE, PTBTag.P_DOLLAR);
...
// ============================= relabel =============================
    ...
    private void labelLightVerbs(CTTree tree, Sentence graph) {
        for (NLPNode node : graph) {
            if (node.getPartOfSpeechTag().startsWith(PTBLib.P_VB) && light_verbs.contains(node.getLemma()))
//			if (tree.getForm(node.getTokenID()-1).isFunctionTag(DDGTag.LV))
            {
                NLPNode obj = node.getFirstChild(n -> n.isDependencyLabel(DDGTag.OBJ));
