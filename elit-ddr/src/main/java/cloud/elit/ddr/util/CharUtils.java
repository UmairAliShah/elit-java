...
//	----------------------------------- Alphabet -----------------------------------	
    ...
    /**
     * @return {@code true} if {@code c == 'a', 'e', 'd', 'o', 'u'}
     */
...
//	----------------------------------- Symbols -----------------------------------
    ...
    public static char generalizeSymbol(char c) {
        if (isCurrency(c)) return CharConst.DOLLAR;
        if (isSingleQuotationMark(c)) return CharConst.SINGLE_QUOTE;
        if (isDoubleQuotationMark(c)) return CharConst.DOUBLE_QUOTE;
        if (isApostrophe(c)) return '\'';
        if (isListMark(c) || isHyphen(c)) return CharConst.HYPHEN;
        return c;
    }
